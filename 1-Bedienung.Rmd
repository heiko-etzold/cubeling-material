...
There is also a code-view, which documents the building process of the cube structure. Here, once again, a variety of actions are possible, which will be discussed in more detail in chapter \@ref(bedienungCode).
...
* Change a variable by one direction with **`change(position, by: (number,number))`**, for example `change(VariableA, by: (1,0))`.
